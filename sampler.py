...
# -------- Sampler for molecule generation tasks --------
class Sampler_mol_condition_clfg(object):
    def __init__(self, config, w=None):
        self.config = config
        self.device = load_device()
        self.params_x, self.params_adj = load_model_params(self.config)
    ...
    def sample(self):
        ...
        self.log_folder_name, self.log_dir, _ = set_log(self.configt, is_train=False)
        self.log_name = f"{self.config.ckpt}-sample-{str(self.config.controller.label['frag'])}"
        ...
        train_smiles, _ = load_smiles(self.configt.data.data)
        test_topK_df_1 = filter_smiles_with_labels(self.config)
...
class Sampler_mol_condition_cldt(object):
    ...
    def sample(self):
        ...
        self.log_folder_name, self.log_dir, _ = set_log(self.configt, is_train=False)
        self.log_name = f"{self.config.ckpt}-sample-{str(self.config.controller.label['target'])}-{str(self.config.controller.label['kd'])}"
        ...
        train_smiles, _ = load_smiles(self.configt.data.data)
        # test_topK_df_1 = filter_smiles_with_labels(self.config, topk=2)
        # test_topK_df_2 = filter_smiles_with_labels(self.config, topk=3)
        # test_topK_df_3 = filter_smiles_with_labels(self.config, topk=4)
        # test_topK_df_4 = filter_smiles_with_labels(self.config, topk=5)
        # test_topK_df_5 = filter_smiles_with_labels(self.config, topk=10)
        ...
        test_smiles_1 = canonicalize_smiles(test_topK_df_1['compound_iso_smiles'].tolist())
        test_smiles_2 = canonicalize_smiles(test_topK_df_2['compound_iso_smiles'].tolist())
        test_smiles_3 = canonicalize_smiles(test_topK_df_3['compound_iso_smiles'].tolist())
        test_smiles_4 = canonicalize_smiles(test_topK_df_4['compound_iso_smiles'].tolist())
        test_smiles_5 = canonicalize_smiles(test_topK_df_5['compound_iso_smiles'].tolist())

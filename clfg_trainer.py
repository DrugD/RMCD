...
class CLFGTrainer(object):
    def __init__(self, config):
        super(CLFGTrainer, self).__init__()
        ...
        self.model = TransFGDRP(self.config.controller, self.params_x, self.params_adj, f'cuda:{self.device[0]}', self.config.train)
    ...
    def train(self, ts):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in tqdm(enumerate(self.train_loader)):
                ...
                logits_per_df, logits_per_text = self.model.forward2gen(x, adj, frag)
                
                # pdb.set_trace()
                labels = torch.arange(frag.shape[0]).long().to(f'cuda:{self.device[0]}')

                loss_dc = cross_entropy_loss(logits_per_df, labels)
            ...
            if epoch % self.config.train.print_interval == self.config.train.print_interval-1:
                tqdm.write(f'[EPOCH {epoch+1:04d}] train clip: {mean_train_CLIP:.3e} | test clip: {mean_test_CLIP:.3e} | ')

...
class MultiDataControlTrainer(object):
    def __init__(self, config):
        super(MultiDataControlTrainer, self).__init__()
        ...
        target_dict = json.load(open(f"data/{self.config.data.data[0]}/proteins.txt"), object_pairs_hook=OrderedDict)
        ...
        self.dta_label_embedding.to('cuda:'+str(self.device[0]))
        self.dta_label_embedding.eval()
    ...
    def train(self, ts):
        ...
        # import pdb;pdb.set_trace()
        # DLE: dta_label_embedding
        # self.optimizer_DLE = torch.optim.Adam(self.dta_label_embedding.parameters(), lr=self.config.train.lr, 
        #                                 weight_decay=self.config.train.weight_decay)
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in enumerate(self.train_loader):
                ...
                loss_subject = (x, adj, self.dta_label_embedding.multi_data_text_forward(label))
                loss_x, loss_adj = self.loss_fn(self.model_x, self.model_adj, *loss_subject)
                
                # loss_subject = (x, adj, self.dta_label_embedding(label))
            ...
            if epoch % self.config.train.save_interval == self.config.train.save_interval-1:
                ...
                # torch.save({ 
                #     'model_config': self.config,
                #     'dta_label_embedding': self.dta_label_embedding.state_dict()

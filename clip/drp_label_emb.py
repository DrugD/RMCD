...
class DRPLabelEmbedding(torch.nn.Module):
    def __init__(self, device):
        super(DRPLabelEmbedding, self).__init__()
        clip.available_models()
        
        config= {"cuda_name":'cuda:'+str(device[0])}
        self.config  =  config
        ...
        self.transformer_fusion_1 = MLP(embed_dim*2, 512, 300)
        self.transformer_fusion_2 = MLP(embed_dim*2, 512, 100*100)
    ...
    def forward(self, data):
        ...
        text = clip.tokenize(descriptions_text,context_length=300).to(self.config['cuda_name'])
        number = clip.tokenize(descriptions_number,context_length=100).to(self.config['cuda_name'])
        # 
        ...
        sentence_features_1 = self.transformer_fusion_1(sentence_features)
        sentence_features_2 = self.transformer_fusion_2(sentence_features)
        # normalized features
        # fusion_features = fusion_features / fusion_features.norm(dim=1, keepdim=True)
        sentence_features_1 = sentence_features_1 / sentence_features_1.norm(dim=1, keepdim=True)
        sentence_features_2 = sentence_features_2 / sentence_features_2.norm(dim=1, keepdim=True)
        # pdb.set_trace()
        ...
        if not self.training:
            condition_emb = sentence_features_1.squeeze(), sentence_features_2.squeeze()
            return condition_emb
        
        mask = self.create_mask(sentence_features_1.unsqueeze(-1),0.3)
        
        sentence_features_1 = sentence_features_1.unsqueeze(-1) * mask.unsqueeze(-1).expand_as(sentence_features_1.unsqueeze(-1)).float()
        sentence_features_2 = sentence_features_2.unsqueeze(-1) * mask.unsqueeze(-1).expand_as(sentence_features_2.unsqueeze(-1)).float()
        # import pdb;pdb.set_trace()
        return sentence_features_1.squeeze(), sentence_features_2.squeeze()
    
    
    def forward_null_text(self, data):
        ...
        # mask = self.create_mask(sentence_features_1.unsqueeze(-1),0.5)

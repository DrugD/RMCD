...
from controller import TransDTA
import pdb
...
class CLDTTrainer(object):
    def __init__(self, config):
        super(CLDTTrainer, self).__init__()
        ...
        target_dict = json.load(open(f"data/{self.config.data.data}/proteins.txt"), object_pairs_hook=OrderedDict)
        self.target_list = list(target_dict.values())

        self.params_x, self.params_adj = load_model_params(self.config)
        self.transe = TransE(1000, 1, f'cuda:{self.device[0]}', dim=128)
        self.model = TransDTA(self.config.controller, self.params_x, self.params_adj, f'cuda:{self.device[0]}', self.config.train, self.target_list, PRECISION=self.config.PRECISION)
        ...
        self.model.get_target_feature(self.config.controller.target_txt_path)
    ...
    def train(self, ts):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in tqdm(enumerate(self.train_loader)):
                ...
                # pdb.set_trace()
                target_name, kd = label[:,:,0], label[:,:,1]
                
          
                logits_per_dc, logits_per_text, num_logits_per_dc, num_logits_per_text = self.model.forward4gen(x, adj, target_name, kd)
                

                number_features = self.model.generate_samples(0, 1*(self.config.PRECISION**2), self.config.data.data, PRECISION=self.config.PRECISION)
                ...
                labels = torch.arange(kd.shape[0]).long().to(f'cuda:{self.device[0]}')

...
class MultiDataControlTrainer(object):
    def __init__(self, config):
        ...
        self.drp_label_embedding = TransEDRP(self.config.controller, self.params_x, self.params_adj, f'cuda:{self.device[0]}', self.config.train)
        self.drp_label_embedding.load_state_dict(torch.load(self.config.controller.cldr_ckpt)['model_state_dict'], strict = True)
    ...
    def train(self, ts):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in enumerate(self.train_loader):
                ...
                loss_subject = (x, adj, self.drp_label_embedding.multi_data_text_forward(label))
            ...
            if epoch % self.config.train.save_interval == self.config.train.save_interval-1:
                ...
                torch.save({ 
                    ...
                    }, f'/home/lk/project/mol_generate/GDSS/checkpoints/{self.config.data.data}/{self.ckpt + save_name}.pth')

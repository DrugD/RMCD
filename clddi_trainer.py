...
from controller import TransDDI
import pdb
from models.transE import TransE
from tqdm import tqdm

class CLDDITrainer(object):
    def __init__(self, config):
        super(CLDDITrainer, self).__init__()
        ...
        self.model = TransDDI(self.config.controller, self.params_x, self.params_adj, f'cuda:{self.device[0]}', self.config.train)
    ...
    def train(self, ts):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in tqdm(enumerate(self.train_loader)):
                ...
                drug2, effect = label[0], label[1]
                logits_per_dc, logits_per_text = self.model.forward4gen(x, adj, drug2, effect)
                # logits_per_dc, logits_per_text, num_logits_per_dc, num_logits_per_text = self.model.forward4gen(x, adj, drug2, effect)
                ...
                labels = torch.arange(effect.shape[0]).long().to(f'cuda:{self.device[0]}')
                ...
                self.train_CLIP.append(loss.item())
            ...
            with torch.no_grad():
                for _, test_b in enumerate(self.test_loader):   
                    ...
                    self.test_CLIP.append(loss.item())
            ...
            logger.log(f'{epoch+1:03d} | {time.time()-t_start:.2f}s | '
                        ...
                        f'train clip: {mean_train_CLIP:.3e} | test clip: {mean_test_CLIP:.3e} | ', verbose=False)
            # -------- Save checkpoints --------
            if epoch % self.config.train.save_interval == self.config.train.save_interval-1:
                save_name = f'_{epoch+1}' if epoch < self.config.train.num_epochs - 1 else ''
